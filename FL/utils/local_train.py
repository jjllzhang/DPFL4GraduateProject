...
from train_and_validation.train_with_dp import train_with_dp, train_with_dp_auto, train_with_dp_perlayer
...
def local_clients_train_with_dp_perlayer_one_epoch(num_of_clients, clients_data_list, clients_model_list, clients_criterion_list, clients_optimizer_list, num_epoch, q, device):
    for i in range(num_of_clients):
        batch_size = math.floor(len(clients_data_list[i]) * q)
        train_dataloader = torch.utils.data.DataLoader(clients_data_list[i], batch_size=batch_size, shuffle=False, drop_last = True)
        model = clients_model_list[i].to(device)
        ...
        for epoch in range(num_epoch):
            train_loss, train_accuracy = train_with_dp_perlayer(model, train_dataloader, optimizer, device)
            # print(f'Epoch: {epoch + 1} |'f' Train Loss: {train_loss:.4f} |'f' Train Accuracy: {train_accuracy:.2f}%')


def local_clients_train_with_dp_auto_one_epoch(num_of_clients, clients_data_list, clients_model_list, clients_criterion_list, clients_optimizer_list, num_epoch, q, device):
    for i in range(num_of_clients):
        batch_size = math.floor(len(clients_data_list[i]) * q)
        train_dataloader = torch.utils.data.DataLoader(clients_data_list[i], batch_size=batch_size, shuffle=False, drop_last = True)
        model = clients_model_list[i].to(device)
        ...
        for epoch in range(num_epoch):
            train_loss, train_accuracy = train_with_dp_auto(model, train_dataloader, optimizer, device)
            # print(f'Epoch: {epoch + 1} |'f' Train Loss: {train_loss:.4f} |'f' Train Accuracy: {train_accuracy:.2f}%')
